...
const { schema: graphQLSchema } = require('./graphql/server')
...
const routerHandler = router.getRequestHandler(
  app, ({ req, res, route, query }) => renderAndCache(req, res, route.page, query)
)
...
const initialize = async () => {
  ...
  server.use('/api/graphql', bodyParser.json(), graphqlExpress({
    schema: Schema,
    pretty: true,
    graphiql: true
  }))
  if (isDev) {
    server.get('/api/graphiq', graphiqlExpress({ endpointURL: '/api/graphql' }))
  }
...
initialize().catch(err => logger.error(err))
